...
# -*- coding: iso-8859-15 -*-
"""
Provides IPython QT4 console widgets.

@author: Richard W. Lincoln
r.w.lincoln _at_ gmail.com
This QT4 widget is based on the original work of Laurent Dufrechou who
adapted Eitan Isaacson"s console for the GTK toolkit to WX.
...
__version__ = 0.1
__author__  = "Richard W. Lincoln"
__email__   = "r.w.lincoln _at_ gmail.com"
__license__ = "BSD"
...
from enthought.pyface.ipshell_nonblocking import NonBlockingIPShell

#class QtNonBlockingIPShell(NonBlockingIPShell):
#    """
#    An NonBlockingIPShell Thread that is QT4 dependent.
#    """
...
class QtConsoleView(QsciScintilla):
    """
...
#        self.EnsureCaretVisible()
        ...
        self.setMarginWidth(3, 10) #text is moved away from border

        #----------------------------------------------------------------------
        #  QtConsoleView interface:
        #----------------------------------------------------------------------
...
#        self.Bind(wx.EVT_KEY_DOWN, self._onKeypress)
    ...
    def buildStyles(self):
        # Define platform specific fonts
...
#            self.SetCaretForeground("BLACK")
            self.setCaretForegroundColor(QtGui.QColor(0,0,0))
            self.ANSI_STYLES = self.ANSI_STYLES_WHITE
        else:
#            self.SetCaretForeground("WHITE")
            self.setCaretForegroundColor("WHITE")
...
#            self.StyleSetSpec(style[0], "fore:%s" % style[1])
    ...
    def asyncWrite(self, text):
        """
        Write given text to buffer in an asynchronous way.
        It is used from another thread to be able to access the GUI.
...
#                    self.SetStyling(len(segments[i+1]),self.ANSI_STYLES[tag][0])
    ...
    def getCurrentLine(self):
        ...
        return self.GetTextRange(
            self.getCurrentPromptStart(), self.getCurrentLineEnd()
        )
...
class IPShellWidget(QtGui.QWidget):
    """
    ...
    def __init__(self, parent, intro=None,
                 background_color="WHITE", add_button_handler=None,
                 qt_ip_shell=None, user_ns={},user_global_ns=None,
                 ):
        """
        Initialize.
        ...
        if qt_ip_shell is not None:
            self.IP = qt_ip_shell
        else:
            self.IP = NonBlockingIPShell(
                argv=[], user_ns=user_ns, user_global_ns=user_global_ns,
...
#            welcome_text+= "TAB       -> Autocompletion\n"
        ...
        self.text_ctrl = QtConsoleView(
            self, self.IP.getPrompt(),
            intro=welcome_text, background_color=background_color
        )
...
#        self.background_option.Bind(wx.EVT_CHECKBOX, self.evtCheckOptionBackgroundColor)
        ...
        mainLayout.addWidget(self.text_ctrl)
...
#        self.text_ctrl.SetFocus()
        ...
        if event.IsChecked():
            self.setCompletionMethod("STC")
        else:
            self.setCompletionMethod("IPYTHON")


    def setCompletionMethod(self, value):
        """ Sets the shell completion method """

        if value not in ["STC", "IPYTHON"]:
            raise ValueError, "Completion method may be 'STC' or 'IPYTHON'"

#        self.options["completion"]["value"]=value
#        self.text_ctrl.setCompletionMethod(self.options["completion"]["value"])
#        self.updateOptionTracker("completion",
#                                 self.options["completion"]["value"])
#        self.text_ctrl.SetFocus()
...
#        self.text_ctrl.SetFocus()
    ...
    def setBackgroundColor(self, value):
        """ Sets the shell background colour """

        if value not in ["BLACK", "WHITE"]:
            raise ValueError, "Background colour must be 'BLACK' or 'WHITE'"

        self.options["background_color"]["value"]=value
        self.text_ctrl.setBackgroundColor(self.options["background_color"]["value"])
        self.updateOptionTracker("background_color",
                                 self.options["background_color"]["value"])
        self.text_ctrl.SetFocus()
...
#    app = PyQT4.QtGui.Application(sys.argv)
#    widget = IPShellWidget()
#
#    sys.exit(app.exec_())
